...
export function VehicleForm({ form, setForm, onSubmit, busy, submitLabel }) {
  ...
  return <form onSubmit={onSubmit}>
    ...
    <div className="two"><Field label="Biltype"><select {...field('vehicle_type')}><option>Termo</option><option>Express</option><option>Standard</option>{form.vehicle_type === 'Sideåpning' && <option>Sideåpning</option>}</select></Field><Field label="Dører / åpning"><select required {...field('door_type')}><option value="" disabled>Velg dørtype</option>{doorTypes.map(type => <option key={type}>{type}</option>)}</select></Field></div>
...
export function VehicleTable({ rows, profile, userId, busy, onAction, onEvents }) {
  ...
  return <div className="table"><table className="vehicle-table"><thead><tr><th>Status</th><th>Transportør</th><th>Reg.nr / sted</th><th>Ledig fra</th><th>Biltype / dører</th><th>Reservasjon / lass</th><th>Handling</th></tr></thead><tbody>
    {rows.map(row => <tr key={row.id} className={row.deleted_at ? 'deleted' : row.status === 'Ledig' ? 'available' : 'reserved'}>
      ...
      <td><b>{row.registration}</b><small>{row.location}</small><small>Retning: {row.direction || '–'}</small></td>
      <td>{formatDate(row.available_at)[0]}<small>kl. {formatDate(row.available_at)[1]}</small></td>
      <td>{row.vehicle_type || '–'}<small className="door-type">{row.door_type || 'Dører: Ikke oppgitt'}</small></td>
