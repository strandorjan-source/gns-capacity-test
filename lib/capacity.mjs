...
export const vehicleTypeTabs = [
  ...
  { id: 'thermo', label: 'Termo', field: 'vehicle_type', value: 'Termo' },
  { id: 'express', label: 'Express', field: 'vehicle_type', value: 'Express' },
  { id: 'standard', label: 'Standard', field: 'vehicle_type', value: 'Standard' },
...
export const blankVehicle = { carrier: '', contact: '', phone: '', registration: '', location: 'Oslo', loading_region: '', date: '', time: '', vehicle_type: 'Termo', door_type: '', direction: 'Nord-Norge', comment: '' };
...
export function vehicleChanges(form) {
  ...
  return {
    ...
    registration: normalizeRegistration(form.registration), location: required(form.location, 'Sted'), loading_region: form.loading_region,
...
export function filterVehicles(rows, { history = false, date = '', query = '', status = '', region = '' } = {}) {
  ...
  const entries = rows.filter(row => Boolean(row.is_history) === history
    ...
    && (!needle || [row.carrier, row.contact, row.registration, row.location, row.loading_region, row.vehicle_type, row.door_type, row.direction, row.reserved_by_name, row.reserved_by_email, row.reservation_comment, row.comment].some(value => String(value || '').toLowerCase().includes(needle))));
...
export const vehicleLabels = { carrier: 'Transportør', contact: 'Kontaktperson', phone: 'Telefon', registration: 'Reg.nr', location: 'Sted', loading_region: 'Landsdel klar for lasting', available_at: 'Ledig fra', vehicle_type: 'Biltype', door_type: 'Dører / tilvalg', direction: 'Retning', comment: 'Kommentar' };
