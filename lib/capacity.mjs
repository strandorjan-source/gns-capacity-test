...
export function filterVehicles(rows, { history = false, date = '', query = '', status = '' } = {}) {
  const needle = query.trim().toLowerCase();
  const entries = rows.filter(row => Boolean(row.is_history) === history
    && (!date || osloDate(row.available_at) === date)
    && (!status || row.status === status)
    && (!needle || [row.carrier, row.contact, row.registration, row.location, row.vehicle_type, row.door_type, row.direction, row.reserved_by_name, row.reserved_by_email, row.reservation_comment, row.comment].some(value => String(value || '').toLowerCase().includes(needle))));
