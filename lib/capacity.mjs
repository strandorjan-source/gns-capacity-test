...
export const blankVehicle = { carrier: '', contact: '', phone: '', registration: '', location: 'Oslo', date: '', time: '', vehicle_type: 'Termo', direction: 'Nord-Norge', comment: '' };
...
export const canDeleteVehicle = (p, userId, row) => isAdmin(p) || Boolean(p?.approved && p.role === 'carrier' && row.owner_user_id === userId && row.status === 'Ledig' && !row.reserved_by && !row.reserved_at);
...
export function vehiclePayload(form, userId) {
  ...
  return {
    owner_user_id: userId, carrier: required(form.carrier, 'Transportør'),
    ...
    available_at: osloDateTime(form.date, form.time), vehicle_type: form.vehicle_type,
    direction: String(form.direction || '').trim().slice(0, 200), comment: String(form.comment || '').trim(),
    status: 'Ledig', reserved_by: null, reserved_at: null,
  };
}
...
export function userMessage(error) {
  ...
  if (error?.code === '23505') return 'Dette registreringsnummeret er allerede meldt inn. Oppdater eller fjern den eksisterende bilen først.';
